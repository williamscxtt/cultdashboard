...
function getMonthDays(date: Date): WeekRow[] {
  ...
  void totalSlots // suppress unused var warning
  return weeks
}
...
export default function CalendarPage() {
  ...
  // Fetch calendar whenever userId or month changes
  const fetchCalendar = useCallback(async (uid: string, month: Date) => {
    if (!uid) return
    setFetching(true)
    setSelectedDay(null)
    setError('')
    try {
      ...
      const res = await fetch(
        `/api/generate-calendar?profileId=${uid}&month=${monthParam}`
      )
  ...
  useEffect(() => {
    if (userId) {
      fetchCalendar(userId, currentMonth)
    }
  }, [userId, currentMonth, fetchCalendar])
  ...
  // Build a lookup: dateStr → CalendarEntry
                ...
                {week.days.map((day, di) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (entry) {
                          setSelectedDay(isSelected ? null : entry)
                        }
                      }}
                      style={{
                        minHeight: 80,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {calendar.map(entry => (
            <ListCard
              ...
              onClick={() =>
                setSelectedDay(prev =>
                  prev?.date === entry.date ? null : entry
                )
              }
            />
          ))}
