...
export default function KnowledgePage() {
  ...
  async function handleAnalyse() {
    ...
    try {
      ...
      let data: { documents?: IngestDoc[]; error?: string }
      try { data = JSON.parse(bodyText) } catch { throw new Error(bodyText.slice(0, 120) || 'Server error') }
          ...
          {filtered.map(doc => {
            ...
            return (
              <Card key={doc.id} style={{ padding: 0, overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {!isExpanded && (
                        <p style={{
                          ...
                          WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', margin: '0 0 8px',
                        }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        >
                          {isExpanded ? <><ChevronUp size={11} /> Collapse</> : <><ChevronDown size={11} /> Read full content</>}
                        </button>
                      </div>
                    </div>
                    <button
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid var(--border)', padding: '14px 16px', background: 'var(--muted)' }}>
                    <pre style={{
