    ...
    <div style={{
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', animation: 'fadeUp 0.45s ease both' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '20px 22px', marginBottom: 32 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 14 }}>
            What&apos;s inside
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
            {FEATURES.map(({ icon: Icon, label }) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.72)', lineHeight: 1.4 }}>{label}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', marginBottom: 5, letterSpacing: '0.02em' }}>Monthly</div>
                    <div style={{ fontSize: 30, fontWeight: 800, color: '#93c5fd', letterSpacing: '-0.6px', lineHeight: 1 }}>
                      £95<span style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255,255,255,0.3)' }}>/mo</span>
                    </div>
                  </div>
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', marginBottom: 5, letterSpacing: '0.02em' }}>6 Months</div>
                    <div style={{ fontSize: 30, fontWeight: 800, color: '#fff', letterSpacing: '-0.6px', lineHeight: 1 }}>
                      £395<span style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255,255,255,0.3)' }}> total</span>
                    </div>
                    <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>~£66/mo · save £175</div>
                  </div>
            ...
            <div style={{ animation: 'fadeUp 0.3s ease both' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', marginBottom: 20 }}>
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', fontSize: 12, cursor: 'pointer', fontWeight: 600, padding: 0, fontFamily: 'inherit' }}
                >
                  Change
                </button>
              </div>
              ...
              {clientSecret && (
                <Elements key={selectedPlan} stripe={stripePromise} options={{ clientSecret, appearance: STRIPE_APPEARANCE }}>
                  <CheckoutForm
                    plan={selectedPlan}
                    onBack={() => { setSelectedPlan(null); setClientSecret(null) }}
                  />
                </Elements>
              )}
            </div>
        ...
        {!selectedPlan && (
          <div style={{ marginTop: 20, padding: '14px 18px', borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', fontSize: 12, color: 'rgba(255,255,255,0.3)', lineHeight: 1.65 }}>
        ...
        {/* ── Footer ── */}
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.18)', lineHeight: 1.6, marginTop: 24, textAlign: 'center' }}>
          Questions before you commit?{' '}
          <a href="https://instagram.com/williamscxtt" style={{ color: 'rgba(255,255,255,0.35)', textDecoration: 'none', fontWeight: 600 }}>
